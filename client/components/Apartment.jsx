...
const Apartments = () => {
    ...
    const handleCreate = async () => {
        const res = await axios.post('http://localhost:8000/apartments',user)
    ...
    const handleDelete = async () => {
        const id = selected.i
        const res = await axios.delete(`http://localhost:8000/apartment/${id}`)
        if (res.data.message) setMessage(res.data.message)
        else setError(res.data.error)
    }
    ...
    <div className='flex items-center h-screen relative'>
        ...
        <section className='mx-auto max-w-[4xl] '>
                    ...
                    {spots.map((d,i) => {
                        ...
                        return(
                            <div key={i} className='px-4 py-2 rounded-sm shadow-sm hover:bg-slate-900 hover:text-white flex items-center justify-between w-full font-semibold'>
                                ...
                                <GridIcon onClick={() => {
                                    setSelected(name,type,price,id)
                                    setEdit(true)
            ...
            <form onSubmit={handleCreate} className='border-2 bg-white shadow-md rounded-sm p-5 grid gap font-semibold w-[400px]'>
                ...
                <XIcon onClick={() =>setCreate(false)} className='cursor-pointer flex w-full justify-end '/>
            ...
            <form onSubmit={handleUpdate} className='border-2 bg-white shadow-md rounded-sm p-5 grid gap font-semibold w-[400px]'>
                <h2 className='text-xl my-2'>Gift - Edit Apartment</h2>
                <XIcon onClick={() =>setEdit(false)} className='cursor-pointer flex w-full justify-end '/>
                <div className='grid gap-3 w-full'>
                {inputs.map((d,i) => {
                    const {name,type,placeholder} = d
                    return(
                ...
                <div className='mt-4'>
                <button type='submit' className='border-2 px-4 py-2 rounded-md border-slate-900 text-slate-900 hover:bg-slate-900 hover:text-white '>Update</button>
